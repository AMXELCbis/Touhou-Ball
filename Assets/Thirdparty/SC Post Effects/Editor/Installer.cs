    ...
    public class Installer : Editor
    {
        ...
        public class ScriptPackages
        {
            ...
            public static bool URPUnpacked()
            {
                ...
                path = AssetDatabase.GUIDToAssetPath("91f16b1b54b30554b8d0074f9d4bab1b");
                file = AssetDatabase.LoadAssetAtPath(path, typeof(UnityEngine.Object));
            ...
            {
                string path = AssetDatabase.GUIDToAssetPath(PPSInstallationMarkerGUID);
                UnityEngine.Object file = AssetDatabase.LoadAssetAtPath(path, typeof(UnityEngine.Object));
                ...
                if (!markerPresent)
                {
                    ...
                    foreach (FileInfo item in fileInfo)
                    {
                        if (item.Name.Contains("PPS_INSTALLED.txt")) markerPresent = true;
                    }
                }

    #if SCPE_DEV
                Debug.Log("PPS installation marker " + (markerPresent ? "" : "NOT") + " present");
    #endif
                return markerPresent;
            }
